...
OpenClaw surfaces are classified as capability surfaces only. `authority_capable` is always `false`; authority must be bound from outside the surface classification.

| OpenClaw surface | `surface_type` | `risk_class` | `closure_state` | `mutation_capable` | `authority_capable` | `required_boundary` | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| filesystem read | `filesystem_read` | `P0_READ_ONLY` | classification-only | `false` | `false` | ATAO visibility boundary | Reads are non-mutating but may expose sensitive state; visibility is not legitimacy. |
| filesystem write | `filesystem_write` | `P2_BOUNDED_MUTATION` | classification-only | `true` | `false` | AEO exact-object validation boundary | Writes require path, content hash, scope, and replay constraints. |
| shell / exec | `shell_exec` | `P4_PRIVILEGED_EXECUTION` | classification-only | `true` | `false` | AEO privileged execution boundary | Shell execution requires command allowlist, environment bounds, timeout, resource limits, and exact-object hash. |
| browser | `browser` | `P3_EXTERNAL_MUTATION` | classification-only | `true` | `false` | AEO external mutation boundary | Browser actions may alter remote state, authenticated sessions, or third-party systems. |
| cron / scheduled action | `scheduled_action` | `P5_AUTONOMOUS_RECURSIVE` | classification-only | `true` | `false` | AEO autonomous recurrence boundary | Scheduled actions can recur beyond the initiating session and require finality and cancellation constraints. |
| gateway routing | `gateway_routing` | `P1_EXECUTION_ADJACENT` | classification-only | `false` | `false` | ATAO routing visibility boundary | Routing affects which boundary receives a request; it must not become implicit authority. |
| session spawn | `session_spawn` | `P5_AUTONOMOUS_RECURSIVE` | classification-only | `true` | `false` | AEO recursive session boundary | Spawning sessions creates delegated continuity surfaces and must be bounded by identity and replay domain constraints. |
| process control | `process_control` | `P4_PRIVILEGED_EXECUTION` | classification-only | `true` | `false` | AEO privileged process boundary | Process start, stop, signal, or lifecycle mutation can affect host-level state. |
| behavioral files | `behavioral_files` | `P1_EXECUTION_ADJACENT` | classification-only | `true` | `false` | ATAO/AEO behavioral configuration boundary | Behavioral files may reshape future agent behavior; mutation requires exact content and scope constraints even when the immediate effect is indirect. |
...
- **#1628 remains open** as the OpenClaw child classification surface.
