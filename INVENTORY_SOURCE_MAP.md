...
### Canonical Sources (two declared, by consumer)

|Consumer                               |Canonical File                              |Role                                   |
|---------------------------------------|--------------------------------------------|---------------------------------------|
|`merge-governance-check.yml` (line 504)|`runtime/surfaces/EXECUTION_SURFACES.json`  |Runtime enforcement surface inventory  |
|`src/index.ts` (line 7464)             |`governance/runtime/EXECUTION_SURFACES.json`|Governance legitimacy surface inventory|


> **Split-source finding:** Two different consumers load from two different files with diverged content and different schemas. This is the primary GAP-004 blocker. Convergence to a single canonical source requires a separate bounded implementation slice (Slice 2) — not this document.

### Non-Canonical Copies

|File                                                                       |Line Count|Classification                |Rationale                                                                                                                                                                                                |
|---------------------------------------------------------------------------|----------|------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|`EXECUTION_SURFACES.json` (root)                                           |253       |`ROOT_ACCUMULATION`           |Largest file; contains `canonical_runtime_route` field absent in both canonical sources; likely accumulated across phases without retirement. Not consumed by any confirmed runtime or workflow consumer.|
|`governance/execution_surfaces.json`                                       |118       |`GOVERNANCE_PROJECTION_LEGACY`|Lowercase filename, version 1.0, subset of `governance/runtime/EXECUTION_SURFACES.json`. No confirmed consumer. Likely superseded by `governance/runtime/` copy.                                         |
|`runtime/execution_surfaces.json`                                          |100       |`RUNTIME_PROJECTION_LEGACY`   |Lowercase filename, subset of `runtime/surfaces/EXECUTION_SURFACES.json`. No confirmed consumer. Likely superseded by `runtime/surfaces/` copy.                                                          |
|`governance/mindshift-validation-bundle/governance/EXECUTION_SURFACES.json`|141       |`GENERATED_BUNDLE_COPY`       |Matches `governance/runtime/EXECUTION_SURFACES.json` line count exactly. Generated by `scripts/regenerate-governance-artifacts.mjs`. Canonical source is `governance/runtime/`.                          |
|`PHASE3_EXECUTION_SURFACE_INVENTORY.json` (root)                           |—         |`PHASE_CLOSURE_EVIDENCE`      |Phase 3 closure artifact. Historical evidence, not current operative inventory.                                                                                                                          |

### Specialized Execution Surface Files

|File                                                        |Classification|Rationale                                                          |
|------------------------------------------------------------|--------------|-------------------------------------------------------------------|
|`governance/runtime/EXECUTION_SURFACE_CLOSURE_REGISTRY.json`|`CURRENT`     |Closure tracking registry, distinct purpose from surface inventory.|
|`governance/runtime/EXECUTION_SURFACE_CLOSURE_REPORT.json`  |`CURRENT`     |Closure report artifact, distinct purpose.                         |
|`governance/runtime/EXECUTION_SURFACE_MAP.json`             |`CURRENT`     |Map/topology artifact, distinct purpose.                           |
|`EXECUTION_SURFACE_CLASSIFICATION.md`                       |`CURRENT`     |Classification vocabulary doc, distinct purpose.                   |
...
|`runtime/legitimacy/schemas/PROOF_OBJECT.schema.json`       |`DEAD_CONTRACT`                                                                       |Removed from schema-validator `SCHEMA_FILES` (issue #1884). Schema describes an object model (`object_type: "ProofObject"`, `result`/`target_system`/`target_action`/`proof_reference`) that no production writer ever built. Runtime proof shape is the `proof_registry` row — see issue #1884 reachability matrix. Schema-validator no longer loads this file; `validateLegitimacySchema` with `object_type: "ProofObject"` returns `UNKNOWN_OBJECT_TYPE`. File retained as historical evidence only.|
...
|---------------------------------------------------|------------------|----------------------------------------------------------------|
|`merge-governance-check.yml`                       |Execution surfaces|`runtime/surfaces/EXECUTION_SURFACES.json`                      |
|`src/index.ts`                                     |Execution surfaces|`governance/runtime/EXECUTION_SURFACES.json`                    |
...
→ runtime/surfaces/EXECUTION_SURFACES.json (226 lines, v1.0.0, generated_at: 2026-05-14)

src/index.ts
→ governance/runtime/EXECUTION_SURFACES.json (141 lines, v1.0, no generated_at)
...
|Slice  |Scope                                                                |Type                  |
|-------|---------------------------------------------------------------------|----------------------|
|Slice 2|Converge `EXECUTION_SURFACES` consumers to single canonical source   |Bounded implementation|
