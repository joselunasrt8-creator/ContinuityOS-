 ...
 *
 * Scans:
 * - package.json scripts for mutation-capable commands
 * - .github/workflows for mutation-capable jobs/steps
 * - TypeScript/JS code for unclassified mutation-capable exports
 *
 * Returns findings with root_cause → potential_consequence mapping.
 */
...
export interface ExecutionSurface {
  surface_id: string;
  type: 'package-script' | 'workflow-job' | 'workflow-step' | 'test-surface' | 'mutation-capable-export';
  name: string;
  location: string;
  mutation_capable: boolean;
  classification: string | null; // @freeze, @test-only, @research-only, null
  declared: boolean;
}

export interface ShadowGuardFinding {
  surface_id: string;
  type: 'package-script' | 'workflow-job' | 'workflow-step' | 'test-surface' | 'mutation-capable-export';
  status: 'PASS' | 'NULL';
  location: string;
  ...
  topology_status: 'declared' | 'undeclared' | 'unknown';
  root_cause: 'declared-and-classified' | 'undeclared-mutation-surface' | 'unclassified-surface';
  potential_consequence?: string;
  human_review: 'PASS' | 'NULL';
  shadow_guard: 'PASS' | 'NULL';
  divergence_class: string | null;
  result: string;
}

export interface ShadowGuardScanResult {
  status: 'PASS' | 'NULL';
  findings: ShadowGuardFinding[];
  summary: {
    total_surfaces: number;
    declared_surfaces: number;
    undeclared_surfaces: number;
    unclassified_surfaces: number;
 ...
 */
const PHASE_A_DECLARED_SURFACES: ExecutionSurface[] = [
...
export class ShadowGuardScanner {
  ...
  private declaredSurfaces: ExecutionSurface[];

  constructor(repoPath: string = '.', declaredSurfaces?: ExecutionSurface[]) {
   ...
   */
  async scan(): Promise<ShadowGuardScanResult> {
   ...
   */
  private generateFinding(surface: ExecutionSurface): ShadowGuardFinding {
    const declared = surface.declared;
    const classified = surface.classification !== null;

    let status: 'PASS' | 'NULL';
    let rootCause: 'declared-and-classified' | 'undeclared-mutation-surface' | 'unclassified-surface';
    let potentialConsequence: string | undefined;
    let divergenceClass: string | null = null;
