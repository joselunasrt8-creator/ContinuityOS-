...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/authority" && request.method === "POST") {
      ...
      if (governed_tool_envelope_id) await env.DB.prepare(`UPDATE governed_tool_envelope_registry SET non_operative='false' WHERE envelope_id=?1`).bind(governed_tool_envelope_id).run()
