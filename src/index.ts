...
import { handleFilesystemWriteRoute } from "./lib/filesystem-write-route-adapter.ts"
import {
  boundedObservabilityWindow,
  emitBootstrapDiagnostic,
  emitInstallBaseTelemetryEvidenceBestEffort,
  emitTelemetry,
  governanceObservabilityEvidence,
  installBaseGovernanceMetrics,
  recordDrift,
} from "./lib/runtime-observability-adapter.js"
import {
  topologyRouteFlags,
  buildRuntimeTopologyReconciliationEnvelope,
  appendRuntimeTopologySnapshot,
  CONSENSUS_DRIFT_TAXONOMY,
  consensusRouteFlags,
  buildGovernanceConsensusEnvelope,
  appendGovernanceConsensusEvidence,
...
// ── Issue #1890 / V2 Phase 2: Filesystem-write route adapter shell ───────────
// Adapter logic extracted to src/lib/filesystem-write-route-adapter.ts.
// This function delegates to the adapter, which owns request parsing, D1 wiring,
// and response formatting while preserving the mandatory kernel chain:
//   intent → ATAO capture → AEO compile → Ω validate → execute → proof

async function handleAgentToolGatewayFilesystemWrite(env: Env, request: Request): Promise<Response> {
  return handleFilesystemWriteRoute(env, request)
}
...
async function latestTopologyReconciliationEvidence(env: Env): Promise<Record<string, unknown>> {
  const regenerated = await buildRuntimeTopologyReconciliationEnvelope(new Date(0).toISOString(), TOPOLOGY_CTX)
...
const TOPOLOGY_CTX: RuntimeTopologyContext = {
  canonical_routes: CANONICAL_RUNTIME_ROUTES,
  observability_routes: NON_EXECUTABLE_OBSERVABILITY_ROUTES,
  governed_support_surfaces: AGENT_TOOL_GOVERNED_SUPPORT_SURFACES,
  workflow_mutation_surfaces: GOVERNED_DEPLOY_WORKFLOW_SURFACES,
  adapter_mutation_surfaces: ADAPTER_MUTATION_SURFACES,
  recursive_governance_containment_registry: RECURSIVE_GOVERNANCE_CONTAINMENT_REGISTRY,
  root_authority_observability_registry: ROOT_AUTHORITY_OBSERVABILITY_REGISTRY,
}

const CONSENSUS_CTX: GovernanceConsensusContext = {
  canonical_routes: CANONICAL_RUNTIME_ROUTES,
  observer_consensus_routes: OBSERVER_CONSENSUS_ROUTES,
  external_conformance_routes: EXTERNAL_CONFORMANCE_ROUTES,
  observer_attestation_registry: OBSERVER_ATTESTATION_REGISTRY,
  semantic_equivalence_registry: SEMANTIC_EQUIVALENCE_REGISTRY,
  portable_governance_checkpoint_registry: PORTABLE_GOVERNANCE_CHECKPOINT_REGISTRY,
  external_conformance_verification_registry: EXTERNAL_CONFORMANCE_VERIFICATION_REGISTRY,
  runtime_id: RUNTIME_ID,
}
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if ([...OBSERVER_CONSENSUS_ROUTES, ...EXTERNAL_CONFORMANCE_ROUTES].includes(url.pathname as any) && request.method === "GET") {
      try {
        ...
        await assertSchemaAvailableReadOnly(env)
        const envelope = await buildGovernanceConsensusEnvelope(url, CONSENSUS_CTX)
    ...
    if ((TOPOLOGY_OBSERVABILITY_ROUTES as readonly string[]).includes(url.pathname)) {
      ...
      const envelope = await buildRuntimeTopologyReconciliationEnvelope(generated_at, TOPOLOGY_CTX)
      await appendRuntimeTopologySnapshot(env, envelope, TOPOLOGY_CTX, new Date().toISOString())
