...
async function ensureSchema(env: Env, options: { stabilizeProofRegistry?: boolean } = {}) {
  ...
  try {
    const stmts = [
      ...
      `CREATE TABLE IF NOT EXISTS invocation_registry (decision_id TEXT NOT NULL, validated_object_hash TEXT NOT NULL, invocation_nonce TEXT NOT NULL, nonce_domain TEXT NOT NULL DEFAULT 'session', nonce_scope TEXT NOT NULL DEFAULT '', nonce_epoch TEXT NOT NULL DEFAULT '', candidate_hash TEXT NOT NULL DEFAULT '', status TEXT NOT NULL, created_at TEXT NOT NULL, continuity_id TEXT, PRIMARY KEY(decision_id, validated_object_hash, invocation_nonce))`,
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_invocation_registry_nonce_domain_scope ON invocation_registry(nonce_domain, nonce_scope, invocation_nonce)`,
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_invocation_registry_nonce_global ON invocation_registry(invocation_nonce)`,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/validate" && request.method === "POST") {
      const b = await body(request); const decision_id = String(b.decision_id || ""); const validated_object_hash = String(b.validated_object_hash || ""); const invocation_nonce = String(b.invocation_nonce || ""); const environment = b.environment; const session_id = String(b.session_id || ""); const nonce_domain = String(b.nonce_domain || "session"); const nonce_scope = String(b.nonce_scope || session_id); const nonce_epoch = String(b.nonce_epoch || ""); const candidate_hash = validated_object_hash
      ...
      const insert = await env.DB.prepare(`INSERT OR IGNORE INTO invocation_registry (decision_id,validated_object_hash,invocation_nonce,nonce_domain,nonce_scope,nonce_epoch,candidate_hash,continuity_id,status,created_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8,'RESERVED',?9)`).bind(decision_id,validated_object_hash,invocation_nonce,nonce_domain,nonce_scope,nonce_epoch,candidate_hash,String(authority.continuity_id || ""),new Date().toISOString()).run()
      // legacy invariant reference retained for conformance source-scans:
      // if ((insert.meta?.changes||0)===0) return rejectWithTelemetry(env, { status:"NULL", result:"INVALID", reason:"nonce_used" }, { event_type: "REPLAY_BLOCKED" })
      if ((insert.meta?.changes||0)===0) {
        const nonceRows = await env.DB.prepare(`SELECT invocation_nonce, nonce_domain, nonce_scope, nonce_epoch, candidate_hash FROM invocation_registry WHERE invocation_nonce=?1 ORDER BY created_at ASC LIMIT 5`).bind(invocation_nonce).all<any>()
        const rows = Array.isArray(nonceRows.results) ? nonceRows.results : []
        const sameDomain = rows.find((row: any) => String(row.nonce_domain || "") === nonce_domain && String(row.nonce_scope || "") === nonce_scope)
        const reason = sameDomain && String(sameDomain.candidate_hash || "") && String(sameDomain.candidate_hash || "") !== candidate_hash
          ? "nonce_candidate_hash_mismatch"
          : sameDomain && String(sameDomain.nonce_epoch || "") && nonce_epoch && String(sameDomain.nonce_epoch || "") !== nonce_epoch
            ? "stale_nonce_epoch"
            : rows.some((row: any) => String(row.nonce_domain || "") !== nonce_domain || String(row.nonce_scope || "") !== nonce_scope)
              ? "cross_domain_nonce_reuse"
              : "nonce_used"
        const replay_classification = reason === "nonce_candidate_hash_mismatch" ? "REPLAY_NONCE_HASH_MISMATCH" : reason === "stale_nonce_epoch" ? "REPLAY_NONCE_EPOCH_STALE" : reason === "cross_domain_nonce_reuse" ? "REPLAY_NONCE_CROSS_DOMAIN" : "REPLAY_NONCE_REUSED"
        return rejectWithTelemetry(env, { status:"NULL", result:"INVALID", reason }, { event_type: "REPLAY_BLOCKED", decision_id, authority_id: String(authority.authority_id || ""), severity: "HIGH", payload: { route: "/validate", validated_object_hash, invocation_nonce, nonce_domain, nonce_scope, nonce_epoch: nonce_epoch || null, indicator: "reused_nonce", replay_classification }, drift_class: "replay_drift" })
      }
