...
| current_state | PARTIAL — GMA (Governance Mutation Authorization) enforcement active: governance_mutation and workflow_mutation class PRs require a valid GMA artifact with canonical lifecycle lineage (/session → /continuity → /authority → /compile). Enforcement in merge-governance-check.yml. Creation workflow: governance-mutation-authorization.yml. Spec: governance/authorizations/GOVERNANCE_MUTATION_AUTHORIZATION_SPEC.json. Bootstrap exemption (GMA_BOOTSTRAP_EXEMPTION) applies only to the single PR introducing GAP-005 GMA infrastructure — self-limiting, non-reusable, auditable. Once merged, all subsequent governance mutations fully enforce GMA. Bounded containment check added (#1831): scripts/governance-mutation-containment-check.mjs + governance-mutation-containment.yml enforce that governance primitive mutations (validator files, schema files, policy files, workflow YAML files, governance registry files) carry a Governance Mutation Proof (GMP) covering the full lifecycle /session → /continuity → /authority → /compile → /validate → /execute → /proof. Fails closed without valid GMP. Tests at tests/fate/issue-1831-governance-mutation-containment.test.mjs (49 cases). Remaining gap: GMP creation workflow (wiring /validate → /execute → /proof against live runtime to produce GMP artifacts). |
...
| fate_tests | tests/fate/issue-1831-governance-mutation-containment.test.mjs |
| remaining_closure | Implement GMP creation workflow: wire /validate → /execute → /proof stages against live runtime; produce and commit GOVERNANCE_MUTATION_PROOF.json alongside GMA; update governance-mutation-containment.yml to enforce GMP presence end-to-end |
| status | PARTIAL |
