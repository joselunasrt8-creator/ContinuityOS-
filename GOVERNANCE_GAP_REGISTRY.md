...
| current_state | PARTIAL — GMA (Governance Mutation Authorization) enforcement active: governance_mutation and workflow_mutation class PRs require a valid GMA artifact with canonical lifecycle lineage (/session → /continuity → /authority → /compile). Enforcement in merge-governance-check.yml. Creation workflow: governance-mutation-authorization.yml. Spec: governance/authorizations/GOVERNANCE_MUTATION_AUTHORIZATION_SPEC.json. Bootstrap exemption (GMA_BOOTSTRAP_EXEMPTION) applies only to the single PR introducing GAP-005 GMA infrastructure — self-limiting, non-reusable, auditable (consumed by PR #1841; absent from main thereafter, so it cannot recur). First successful end-to-end issuance recorded 2026-06-04: GMA-gap-005-enforcement-56Hxx-1 (governance/authorizations/GOVERNANCE_MUTATION_AUTHORIZATION.json), traversed /session → /continuity → /authority → /compile, validated and merge-gated for PR #1849 (commit 151beeb). This supersedes any prior "0 successful issuances" framing; reproducibility against a second, independent governance mutation remains unexercised since no governance_mutation/workflow_mutation-classified PR has landed since. Remaining gap: runtime proof persistence (/execute → /proof) for governance mutations not yet enforced — the proof recorded for PR #1849 (PROOF-1849-151beeba) is a generic merge_proof_registry entry, not a dedicated governance_mutation_proof. |
