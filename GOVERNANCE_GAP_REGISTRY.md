...
| current_state | CLOSED — GMA (Governance Mutation Authorization) enforcement active: governance_mutation and workflow_mutation class PRs require a valid GMA artifact with canonical lifecycle lineage (/session → /continuity → /authority → /compile). Enforcement in merge-governance-check.yml. Creation workflow: governance-mutation-authorization.yml. Spec: governance/authorizations/GOVERNANCE_MUTATION_AUTHORIZATION_SPEC.json. Bootstrap exemption (GMA_BOOTSTRAP_EXEMPTION) applies only to the single PR introducing GAP-005 GMA infrastructure — self-limiting, non-reusable, auditable. Runtime proof-binding layer now closes the remaining wedge: POST /proof additively detects governance_mutation_authorization lineage, builds a fail-closed GovernanceMutationProof via src/lib/governance-mutation-proof.ts, and persists it to governance_mutation_proof_registry (schema.sql, migrations/0071_governance_mutation_proof_registry.sql) only after the underlying execution proof is committed and authority is consumed. The artifact is lineage-bound to session_id, continuity_id, decision_id, authority_id, gma_id, compiled_object_hash, and executed_object_hash (spec: governance/authorizations/GOVERNANCE_MUTATION_PROOF_SPEC.json). Fail-closed: missing inputs, compiled/executed hash mismatch, invalid GMA status, GMA lineage mismatch, or replayed lineage all collapse to NULL — proven by tests/governance-mutation-proof-binding.test.mjs and tests/governance-mutation-proof-runtime-wiring.test.mjs. Specified → Authorized → Compiled → Executed → Proven is now enforced end-to-end for governance mutations. |
| issue_reference | #1831 |
| required_tests | governance mutation validation, recursive approval lineage, policy drift invalidation, governance mutation proof fail-closed binding (positive + negative) |
| required_proofs | governance lineage persistence, immutable governance mutation evidence, governance_mutation_proof lineage-bound persistence in governance_mutation_proof_registry |
| remaining_closure | none — /execute → /proof stage wired for governance mutations; governance_mutation_proof persisted to governance_mutation_proof_registry with replay/lineage-binding triggers |
| status | CLOSED |
